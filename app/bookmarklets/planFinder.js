(function(bs) {
    (function(e, a, g, h, f, c, b, d) {
        ...
        var courseDict = {};
        var done = false;
        $("table table:nth-child(2)").find("tr").each(function(i) {
            if (done || (i == 0)) return;
            if (!$(this).find("select").length && i > 0) {
                $(this).find('table font').each(function(i) {
                    var s = $(this).text();
                    var courseCode = s.trim().split(' ')[0];
                    var date = s.replace(courseCode, '').trim();
                    courseDict[courseCode].examDate = date;
                });
                done = true;
                return;
            }
            var courseCode = $(this).find("td:first-child font").text().trim();
            ...
            $(this).find("select option").each(function() {
                if ($(this).attr("value") && ($(this).attr("value") != "null"))
                    courseDict[courseCode].indexes.push($(this).attr("value"));
            });
            $(this).find("select").each(function() {
                if ($(this).val() != "null") 
                    courseDict[courseCode].selected = $(this).val();
            });

            if (!courseDict[courseCode].indexes.length)
                courseDict[courseCode].indexes = [""];
        });
        
        var str = JSON.stringify(courseDict);
        var h = md5(str);
